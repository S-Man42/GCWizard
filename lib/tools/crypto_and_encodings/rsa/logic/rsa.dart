...
BigInt phi(BigInt p, BigInt q) {
  if (!isPrime(p)) throw FormatException('rsa_error_p.not.prime');

  if (!isPrime(q)) throw FormatException('rsa_error_q.not.prime');
...
List<BigInt>? encryptRSA(List<BigInt> input, BigInt e, BigInt p, BigInt q) {
  ...
  if (!validateE(e, p, q)) throw FormatException('rsa_error_phi.e.not.coprime');
...
BigInt? calculateD(BigInt? e, BigInt? p, BigInt? q) {
  ...
  if (p > BigInt.from(10000) || p > BigInt.from(10000)) throw FormatException('rsa_error_primes.too.large');
  ...
  if (i >= phiN) throw FormatException('rsa_error_could.not.calc.d');
...
List<BigInt>? decryptRSA(List<BigInt> input, BigInt d, BigInt p, BigInt q) {
  ...
  return input.map((number) {
    ...
    if (!validateD(d, p, q)) throw FormatException('rsa_error_phi.d.not.coprime');
