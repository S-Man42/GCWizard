...
class GCWGalleryCheckbox extends StatefulWidget {
  final List<GCWImageViewData> imageData;
  final Function onDoubleTap;

  const GCWGalleryCheckbox({Key key, @required this.imageData, this.onDoubleTap})
      : super(key: key);

  @override
  _GCWGalleryCheckboxState createState() => _GCWGalleryCheckboxState();
}

class _GCWGalleryCheckboxState extends State<GCWGalleryCheckbox> {
  ...
  Widget build(BuildContext context) {
    ...
    widget.imageData.asMap().forEach((index, element) {
      MemoryImage _img;
      try {
        if (widget.imageData[index] != null)
          _img = MemoryImage(widget.imageData[index].bytes);
    ...
    if (_currentImageIndex >= _validImages.length)
      _currentImageIndex = 0;

    if (_validImages.length == 0)
      return Container();
    ...
    return Column(children: <Widget>[
      Stack(
        children: [
          Positioned(
            top: 145,
            child: GCWIconButton(iconData: Icons.arrow_back_ios,
          ...
          Positioned(
            top: 145,
            right: 0,
            child: GCWIconButton(iconData: Icons.arrow_forward_ios,
              size: IconButtonSize.SMALL,
              onPressed: ()  {
                setState(() {
                  _currentImageIndex = modulo(_currentImageIndex + 1, _validImages.length);
