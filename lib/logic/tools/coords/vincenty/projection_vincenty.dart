...
LatLng projectionVincenty(LatLng coord, double bearing, double dist, Ellipsoid ells) {
  ...
  while ((_sigma-_sigmaP).abs() > epsilon && ++_iterLimit < 100) {
    ...
    double _deltaSigma = _B * _sinSigma * (_cos2SigmaM + _B * 0.25 * (_cosSigma * (-1.0 + 2.0 * _cos2SigmaSq)-
  ...
  double _lat2 = atan2(_sinU1 * _cosSigma + _cosU1 * _sinSigma * _cosAlpha1,
      (1.0 - ells.f) * sqrt(_sinAlpha * _sinAlpha + _tmp * _tmp));
  ...
  double _L = _lambda - (1.0 - _C) * ells.f * _sinAlpha *
