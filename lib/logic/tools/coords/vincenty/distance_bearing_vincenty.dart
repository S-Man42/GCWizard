...
DistanceBearingData distanceBearingVincenty(LatLng coord1, LatLng coord2, Ellipsoid ells) {
  ...
  do {
    ...
    _sinSigma = sqrt((_cosU2 * _sinLambda) * (_cosU2 * _sinLambda) +
        (_dCosU1SinU2 - _dSinU1CosU2 * _cosLambda) *
            (_dCosU1SinU2 - _dSinU1CosU2 * _cosLambda));
    ...
    if (_cos2SigmaM.isNaN)
      _cos2SigmaM = 0.0; // equatorial line: cosSqAlpha=0
    
    _C = ells.f / 16.0 * _cosSqAlpha *
        (4.0 + ells.f * (4.0 - 3.0 * _cosSqAlpha));
    _lambdaP = _lambda;
    _lambda = _L + (1.0 - _C) * ells.f * _sinAlpha *
        (_sigma + _C * _sinSigma * (_cos2SigmaM +
            _C * _cosSigma * (-1.0 + 2.0 * _cos2SigmaM * _cos2SigmaM)));
  } while ((_lambda - _lambdaP).abs() > epsilon && ++_iterLimit < 40);

  double uSq = _cosSqAlpha * (_a * _a - ells.b * ells.b) /
      (ells.b * ells.b);
  double A = 1.0 +
      uSq / 16384.0 * (4096.0 + uSq * (-768.0 + uSq * (320.0 - 175.0 * uSq)));
  double B = uSq / 1024.0 *
      (256.0 + uSq * (-128.0 + uSq * (74.0 - 47.0 * uSq)));
  double deltaSigma = B * _sinSigma * (_cos2SigmaM +
      B / 4.0 * (_cosSigma * (-1.0 + 2.0 * _cos2SigmaM * _cos2SigmaM) -
          B / 6.0 * _cos2SigmaM * (-3.0 + 4.0 * _sinSigma * _sinSigma) *
  ...
  var _bearingBToA =
      pi + atan2(_cosU1 * _sinLambda, -_dSinU1CosU2 + _dCosU1SinU2 * _cosLambda);

  if (_bearingBToA < 0.0)
    _bearingBToA = (pi * 2) + _bearingBToA;

  if (_bearingAToB < 0.0)
    _bearingAToB = (pi * 2) + _bearingAToB;
