...
Zone getZoneByCode(String code) {
  _XY xy = getXYByCode(code);
  int level = code.length - 2;
  Zone zone = _getZoneByXY(xy.x, xy.y, level);
  return zone;
}

_XY getXYByLocation(double lat, double lon, int level) {
...
Zone _getZoneByXY(double x, double y, int level) {
  double h_size = _calcHexSize(level);
  int h_x = x.round();
  int h_y = y.round();
  double unit_x = 6 * h_size;
