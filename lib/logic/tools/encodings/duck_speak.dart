...
final binaryToDuck = {
  '0' : 'Nak', '1' : 'Nanak', '2' : 'Nananak', '3' : 'Nanananak', '4' : 'Nak?', '5' : 'nak?', '6' : 'Naknak', '7' : 'Naknaknak',
  '8' : 'Nak.', '9' : 'Naknak.', '10' : 'Naknaknaknak', '11' : 'nanak', '12' : 'naknak', '13' : 'nak!', '14' : 'nak.', '15' : 'naknaknak',
};
final duckToBinary = switchMapKeyValue(binaryToDuck);
...
String encodeDuckSpeak(text) {
  ...
  text.codeUnits.forEach((ascii) {
    ...
    out.add(binaryToDuck[convertBase(binary.substring(0, 4), 2, 10)] ?? '');
    out.add(binaryToDuck[convertBase(binary.substring(4, 8), 2, 10)] ?? '');
  });
  return out.join(' ');
}
...
String decodeDuckSpeak(text) {
  ...
  text.split(' ').forEach((nak) {
    decimal.add(int.tryParse(duckToBinary[nak] ?? '0'));
  });
  var binary = [];
