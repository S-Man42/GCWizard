...
final letterFrequency_alphabetEnglish1 = { 'A': 8 , 'B': 2 , 'C': 3 , 'D': 4 , 'E': 12 , 'F': 2 , 'G': 2 , 'H': 6 , 'I': 6 , 'J': 1 , 'K': 1 , 'L': 4 , 'M': 2 , 'N': 6 , 'O': 7 , 'P': 2 , 'Q': 1 , 'R': 6 , 'S': 6 , 'T': 9 , 'U': 3 , 'V': 1 , 'W': 2 , 'X': 1 , 'Y': 2 , 'Z': 1 };
final letterFrequency_alphabetSpanish2 = { 'A': 12 , 'B': 1 , 'C': 4 , 'D': 5 , 'E': 13 , 'F': 1 , 'G': 1 , 'H': 1 , 'I': 6 , 'J': 1 , 'K': 1 , 'L': 5 , 'M': 3 , 'N': 6 , 'Ñ': 1 , 'O': 8 , 'P': 2 , 'Q': 1 , 'R': 7 , 'S': 8 , 'T': 4 , 'U': 4 , 'V': 1 , 'W': 1 , 'X': 1 , 'Y': 1 , 'Z': 1 };
final letterFrequency_alphabetRussian1 = { 'А': 7 , 'Б': 2 , 'В': 4 , 'Г': 1 , 'Д': 3 , 'Е': 9 , 'Ё': 1 , 'Ж': 1 , 'З': 1 , 'И': 7 , 'Й': 1 , 'К': 3 , 'Л': 5 , 'М': 3 , 'Н': 7 , 'О': 11 , 'П': 2 , 'Р': 4 , 'С': 5 , 'Т': 6 , 'У': 2 , 'Ф': 1 , 'Х': 1 , 'Ц': 1 , 'Ч': 1 , 'Ш': 1 , 'Щ': 1 , 'Ъ': 1 , 'Ы': 2 , 'Ь': 2 , 'Э': 1 , 'Ю': 1 , 'Я': 2 };
final letterFrequency_alphabetPolish1 = { 'A': 8 , 'Ą': 1 , 'B': 2 , 'C': 4 , 'Ć': 1 , 'D': 3 , 'E': 8 , 'Ę': 1 , 'F': 1 , 'G': 1 , 'H': 1 , 'I': 9 , 'J': 2 , 'K': 3 , 'L': 2 , 'Ł': 2 , 'M': 3 , 'N': 6 , 'Ń': 1 , 'O': 7 , 'Ó': 1 , 'P': 3 , 'R': 4 , 'S': 4 , 'Ś': 1 , 'T': 4 , 'U': 2 , 'W': 4 , 'Y': 4 , 'Z': 5 , 'Ź': 1 , 'Ż': 1 } ;
final letterFrequency_alphabetGreek1 = { 'Α': 13 , 'Β': 1 , 'Γ': 2 , 'Δ': 2 , 'Ε': 9 , 'Ζ': 1 , 'Η': 5 , 'Θ': 1 , 'Ι': 9 , 'Κ': 4 , 'Λ': 2 , 'Μ': 3 , 'Ν': 6 , 'Ξ': 1 , 'Ο': 9 , 'Π': 4 , 'Ρ': 4 , 'Σ': 7 , 'Τ': 8 , 'Υ': 4 , 'Φ': 1 , 'Χ': 1 , 'Ψ': 1 , 'Ω': 2 };
final letterFrequency_alphabetGreek2 = { 'Ά': 2 , 'Α': 11 , 'Β': 1 , 'Γ': 2 , 'Δ': 2 , 'Ε': 7 , 'Έ': 2 , 'Ζ': 1 , 'Η': 3 , 'Ή': 2 , 'Θ': 1 , 'Ι': 7 , 'Ί': 2 , 'Κ': 4 , 'Λ': 2 , 'Μ': 3 , 'Ν': 6 , 'Ξ': 1 , 'Ο': 7 , 'Ό': 2 , 'Π': 4 , 'Ρ': 4 , 'Σ': 7 , 'Τ': 8 , 'Υ': 3 , 'Ύ': 1 , 'Φ': 1 , 'Χ': 1 , 'Ψ': 1 , 'Ω': 1 , 'Ώ': 1 };



HomophonOutput encryptHomophon (String input, KeyType keyType, Alphabet alphabet, int rotation, int multiplier, String ownKeys) {
  if (input == null || input == '')
    return HomophonOutput('', '', ErrorCode.OK);
  ...
  if (keyType == KeyType.OWN) {
    ...
    if (ownKeyList == null)
      return HomophonOutput('', '', ErrorCode.OWNKEYCOUNT);
  }

  table = _getTable(alphabet, keyType, rotation, multiplier, ownKeyList);

  if (table == null)
    return HomophonOutput('', '', ErrorCode.TABLE);

  input = input.toUpperCase();
  input = input
    .split ("")
    .map((character) => table.containsKey(character) ? character : '')
    .join();
...
HomophonOutput decryptHomophon (String input, KeyType keyType, Alphabet alphabet, int rotation, int multiplier, String ownKeys) {
  if (input == null || input == '')
    return HomophonOutput('', '', ErrorCode.OK);
  ...
  output = input
   .split (" ")
   .map((number) {return _numberToChar(number, table);})
   .join();
...
List<int> _ownKeyList(String ownKeys) {
  ...
  if (ownKeysList.length != 100)
    return null;

  return ownKeysList;
}

Map<String, List<int>> _getTable(Alphabet alphabet, KeyType keyType, int rotation, int multiplier, List<int> ownKeyList) {
...
Map<String, List<int>> _createTable(Map<String, int> letterFrequency, KeyType keyType, int rotation, int multiplier, List<int> ownKeyList) {
