...
int gcCodeToID(String gcCode) {
  ...
  if (gcCode.length <= 3 ||
      (gcCode.length == 4 && gcCode.startsWith(RegExp('[0-9A-F]')))
  ) {                  //hex
    if (gcCode.contains(RegExp('[G-Z]')))
      throw FormatException(_NO_VALID_GC_CODE);

    return int.tryParse(convertBase(gcCode, 16, 10));
  } else {
    if (gcCode.contains(RegExp('[ILOSU]')))
      throw FormatException(_NO_VALID_GC_CODE);
...
String idToGCCode(int id) {
  ...
  if (id < 65536)
    return 'GC' + convertBase('$id', 10, 16);
